...
# GITHUB : https://github.com/aaaaaaaxlmontreal
# ENJOY THANK YOU GOD 💜
...
def delete_line(path_file, check_path):
    ...
    line_numbers = [int(num.strip()) for num in lines_to_delete.split(',') if num.strip().isdigit()]
    if not line_numbers:
...
def auto_comment():
    ...
    if ValueError:
        comments = [...]  
        processed_comments = [comment.replace('&&', '\n') for comment in comments]
        print(f'''{red}  Error: Please enter a valid number for the limit.''')
        return None
    if limit > len(access_tokens):
...
def auto_share():
    ...
    def extract_fb_pages(poo):
        ...
        data = None.json()
        pages = data['data']
        access_tokens = [{'accessToken': page['access_token']} for page in pages]
